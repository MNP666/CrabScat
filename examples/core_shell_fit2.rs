use crabscat::{CoarseFineSearch, FitOptions, Profile, Result, SingleParticleModel, form_factors::CoreShell, write_fit_result};

fn main() -> Result<()> {
    let q: Vec<f64> = (0..100).map(|x| x as f64 /1000.0*5.0).collect();
    let ff = CoreShell::new(
        1.0,
        4.0,
        38.0,
        3.0
    ...
    let observed = model.evaluate(&q)? // returns I(q)
    ...
    let input = FitOptions::new(
        5.0,
        50.0,
        10
    )?;

    let search = CoarseFineSearch::new(
        input,
        4,
        2.0
    )?;

    let best_fit = search.fit(
        &target, 
        |core_value| {
        let ff2 = CoreShell::new(
            1.0,
            4.0,
            core_value,
            3.0)?;
        let fit_profile = SingleParticleModel::new(
            ff2, 100.0, 0.5)?;
        fit_profile.evaluate(&q)})?;
